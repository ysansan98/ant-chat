...
export async function getAllAvailableModels(): Promise<AllAvailableModelsSchema[]> {
  const data = await getDb().query.serviceProviderTable.findMany({
    columns: {
      id: true,
      name: true,
      baseUrl: true,
...
export async function getModelsByServiceProviderId(providerServiceId: string): Promise<ServiceProviderModelsSchema[]> {
  const models = await getDb().select().from(serviceProviderModelsTable).where(eq(serviceProviderModelsTable.serviceProviderId, providerServiceId)).orderBy(desc(serviceProviderModelsTable.createdAt))

  return models
}

export async function setModelEnabledStatus(id: string, status: boolean) {
  return getDb().update(serviceProviderModelsTable).set({ isEnabled: status }).where(eq(serviceProviderModelsTable.id, id)).returning().get()
}
...
export async function addServiceProviderModel(config: AddServiceProviderModelSchema) {
  ...
  const count = await getDb().$count(serviceProviderModelsTable, and(
  ...
  return getDb().insert(serviceProviderModelsTable).values({ ...data, isEnabled: true }).returning().get()
}

export async function deleteServiceProviderModel(id: string) {
  return getDb().delete(serviceProviderModelsTable).where(eq(serviceProviderModelsTable.id, id))
}

export async function getModelById(id: string) {
  return getDb().select().from(serviceProviderModelsTable).where(eq(serviceProviderModelsTable.id, id)).get()
}
