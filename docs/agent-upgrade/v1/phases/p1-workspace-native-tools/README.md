...
| `glob_files`  | 基于 ripgrep 按 glob pattern 查找文件，默认遵守 ignore 规则并限制返回数量。                          | L0    |
| `grep_files`  | 基于 ripgrep 按正则搜索文件内容，支持 `path/include` 限定范围并限制返回数量。                        | L0    |
...
- `glob_files` 和 `grep_files` 必须基于 ripgrep 实现，优先调用随应用打包的 `rg`；未打包时可回退到系统 `rg`，找不到 `rg` 时返回 `AGENT_TOOL_EXEC_FAILED`。
- `glob_files` 使用 `rg --files` 加 glob 过滤；`grep_files` 使用 `rg` 正则搜索。
- `glob_files` 和 `grep_files` 默认遵守 ripgrep 的 ignore 行为，包括工作区内 `.gitignore`、`.ignore`、`.rgignore`；不读取全局 gitignore。
- 隐藏文件默认不返回；`node_modules`、`.git`、`dist`、`build` 默认排除，除非 `path/include` 明确收窄到这些目录。
