...
### TC-P1-9A：搜索 ignore 规则
- 对应验收：AC-P1-5
- 步骤：在工作区创建 `.gitignore`、`.ignore`、隐藏文件、`node_modules`、`.git`、`dist`、`build` 目录，调用 `glob_files/grep_files`。
- 期望：默认遵守工作区内 `.gitignore/.ignore`，不读取全局 gitignore；隐藏文件可见；默认排除 `node_modules/.git/dist/build`。
