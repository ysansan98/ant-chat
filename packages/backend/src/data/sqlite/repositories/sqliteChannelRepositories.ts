...
export class SqliteChannelAccountRepository implements ChannelAccountRepository {
  ...
  async upsert(account: ChannelAccount) {
    this.db.prepare(`INSERT INTO channel_accounts (id, channel_type, display_name, credential_ref, owner_user_id, default_workspace_path, permission_mode, enabled, status, last_error, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      ON CONFLICT(id) DO UPDATE SET channel_type=excluded.channel_type, display_name=excluded.display_name, credential_ref=excluded.credential_ref, owner_user_id=excluded.owner_user_id,
...
export class SqliteChannelReceiptRepository implements ChannelReceiptRepository {
  ...
  async updateExternalMessageId(id: string, externalMessageId: string) { this.db.prepare('UPDATE channel_message_receipts SET external_message_id = ?, updated_at = ? WHERE id = ?').run(externalMessageId, Date.now(), id); return mapReceipt(this.db.prepare('SELECT * FROM channel_message_receipts WHERE id = ?').get(id) as ReceiptRow) }
}

interface AccountRow { id: string, channel_type: ChannelType, display_name: string, credential_ref: string, owner_user_id: string | null, default_workspace_path: string | null, permission_mode: ChannelAccount['permissionMode'], enabled: number, status: ChannelAccount['status'], last_error: string | null, created_at: number, updated_at: number }
