...
describe('parseEvidence · policy-decision', () => {
  it('解析白名单命中的放行判断，含权限模式与判定依据', () => {
    const view = parseEvidence(spanItem('policy-decision', 'allow'), evidence(
      spanStarted('policy-decision', { toolName: 'write', input: { path: '/a' }, operationType: 'write', scope: 'workspace', policy: 'require_approval', basis: 'default.require-approval', mode: 'strict', step: 1, toolCallId: 'c1' }),
      spanCompleted('policy-decision', 'allow', { status: 'allow', outcome: 'allow', whitelist: { matchKey: '/a', entry: { toolName: 'write' } } }),
    ))
    ...
    expect(view.basis).toBe('default.require-approval')
