...
class ConversationTitleService {
  ...
  async initializeProvider(providerId: string) {
    const provider = getAgentRuntimeEnvironment().appDataServices.providerSettingsRepository.getProviderServiceById(providerId)
    if (!provider) {
...
export async function handleInitConversationTitle(options: handleInitConversationTitleOptions) {
  ...
  const modelInfo = getAgentRuntimeEnvironment().appDataServices.providerSettingsRepository.getModelById(modelId)
  if (!modelInfo) {
    throw new Error(`Model not found for id: ${modelId}`)
  }

  const serviceProvider = getAgentRuntimeEnvironment().appDataServices.providerSettingsRepository.getServiceProviderByModelId(modelId)
  if (!serviceProvider) {
    throw new Error(`ServiceProvider not found for modelId: ${modelId}`)
  }

  const messages = await getAgentRuntimeEnvironment().appDataServices.messageService.listByConversation(conversationsId)
